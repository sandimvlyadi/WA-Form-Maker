...
class WafmReceptionNumbersClass {

	function _get($data = array())
    {
    	$q = "SELECT * FROM `wafm_number` ";
    ...
    function edit( $id = 0 ) {
        ...
        $q = "SELECT * FROM `wafm_number` WHERE `id` = '". $id ."' AND `deleted_at` IS NULL;";
        $r = $wpdb->get_results( $q );
        if (count($r) > 0) {
            $result['result'] = true;
            $result['data'] = $r[0];
    ...
    function select( $id = 0 ) {
        ...
        $q = '';
        if ($id == 0) {
        	$q = "SELECT * FROM `wafm_number` WHERE `deleted_at` IS NULL;";
        } else{
        	$q = "SELECT * FROM `wafm_number` WHERE `id` = '". $id ."' AND `deleted_at` IS NULL;";
        }
